...
plan <- drake_plan(out_prj = 4326, # wgs84 lat/lon
                   ...
                   roads_rdf = target(mint_feature(subject = paste0(roads_base_uri, roads$LINEARID), 
                   ...
                   target(rdf_serialize(rdf_parse(roads_rdf3, format = "turtle"), doc = roads_out_file), file_out(roads_out_file)),
                   gages_out_file = "output/gages.rdf",
                   target(rdf_serialize(rdf_parse(gages_rdf3, format = "turtle"), doc = gages_out_file), file_out(gages_out_file)),
                   gages_roads_out_file = "output/gages_roads.rdf",
                   target(rdf_serialize(rdf_parse(gages_roads_rdf, format = "turtle"), doc = gages_roads_out_file), file_out(gages_roads_out_file)))
